...
def Ontogenia(parameters,cqs,ontology):
   ...
   # print('starting with parameters:' + str(parameters))
    ...
    if parameters[len(parameters)-1] == -1:
        ...
        if len(parameters[2]) > len(parameters[3]):
            ...
            fragment = call_generator(parameters[1],user_prompt+str(parameters[3]), (f"""Define an ontology using an overall procedure based on understanding each competency 
                                question using this procedure: {ontogenia}   basing on the procedure, and following the previous output 
                                {read_file_as_string(f'data/ontologies/{ontology}.txt')}
